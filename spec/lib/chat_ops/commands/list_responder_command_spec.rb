RSpec.describe ChatOps::Commands::ListResponderCommand do
  ...
  describe '.run' do
    ...
    it "should list a single responder" do
      ...
      expect(process("incident responders")).to return_response_matching /#{prevent_highlights(user1.name)}/
    end
    ...
    it "should list multiple responders" do
      ...
      incident.save
      expect(process("incident responders")).to return_response_matching(/#{prevent_highlights(user1.name)}/, /#{prevent_highlights(user2.name)}/, /#{incident.incident_id}/)
    end
  end
end
